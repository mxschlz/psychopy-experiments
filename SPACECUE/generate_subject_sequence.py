...
def precompute_sequence(subject_id, block, settings, logging_level="INFO", compute_snr=False):
    ...
    if int(subject_id) > 100:
        # TODO: trial-wise logic here
        raise NotImplementedError
    elif int(subject_id) < 100:
        pass
    ...
    for block in range(block, n_blocks):
        ...
        def insert_pseudo_randomized_cues(df, blockwise=True):
            if not blockwise:
                # TODO: implement trial-wise logic for subjects > 100
                raise NotImplementedError
            if blockwise:
                insert_distractor_cue = None
